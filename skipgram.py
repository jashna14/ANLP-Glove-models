...
def forward_backward_propagation(main_word, context_words, negsamples, W_input, W_output, learning_rate, vocab_size):
	...
	prediction = h.dot(W_output[:,targets])
	...
	del_W_input = pred_error.dot(W_output[:,targets].T)
	# print(del_W_input.shape)

	# print(h.shape)

	del_W_output = np.outer(pred_error,h)

	# print(del_W_output.shape)

	# print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')

	W_input[main_word] = W_input[main_word] - learning_rate*del_W_input


	W_outputt = W_output.T
	for index in range(len(targets)):
		W_outputt[targets[index]] = W_outputt[targets[index]] - learning_rate*del_W_output[index]

	W_output = W_outputt.T	
...
def train_model(parameters):
	...
	W_output = np.random.randn(embedding_len, vocab_size)
	...
	for epoch in range(0,epochs):
		...
		x = "skipgram_epoch" + str(epoch) + ".json"
		out_file = open(x,'w')
...
parameters = define_parameters(3, 0.03, 0.0001, 10, 100, 10, 3)
